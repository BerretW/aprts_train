...
local function PushMenu(onSelect, onBack)
    table.insert(MenuCallbackStack, { onSelect = onSelect, onBack = onBack })
end
...
RegisterNUICallback('menuSelect', function(data, cb)
    ...
    if menu and menu.onSelect then
        Citizen.CreateThread(function() menu.onSelect(data) end)
    end
    MenuOpen = false
end)
...
RegisterNUICallback('menuBack', function(data, cb)
    ...
    if menu and menu.onBack then
        Citizen.CreateThread(function() menu.onBack() end)
    end
end)
...
function HandleDrivingAction(action, value)
    if not DrivingTrainCfg then return end

    if action == 'setSpeed' then
        DrivingSpeed = value or 0
        MaxSpeedCalc(DrivingSpeed)
        SendNUIMessage({ type = 'updateDriving', speed = DrivingSpeed })
    ...
    elseif action == 'forward' then
        ...
        while ForwardActive do
            ...
            if DrivingSpeed >= 1 then
                Citizen.InvokeNative(0xDFBA6BBFF7CCAFBB, MyTrain, DrivingSpeed + 0.1)
            end
        end
    ...
    elseif action == 'backward' then
        ...
        while BackwardActive do
            ...
            if DrivingSpeed >= 1 then
                Citizen.InvokeNative(0xDFBA6BBFF7CCAFBB, MyTrain, -1 * (DrivingSpeed + 0.1))
            end
        end
    ...
    elseif action == 'toggleCamera' then
        CameraMode = value  -- parametr funkce HandleDrivingAction(action, value)
        -- if CameraMode then
        --     SetNuiFocus(false, false)
        -- else
        --     SetNuiFocus(true, true)
...
function BuyMenu(myTrains, station)
    ...
    if #myTrains <= 0 then
        ...
        for train, trainCfg in pairs(Trains) do
            ...
            for _, d in pairs(myTrains) do
                if trainCfg.model == d.trainModel then insert = false end
            end
            if insert then
                local wagons = "Engine: " .. trainCfg.Train ..
                    " | People: " .. trainCfg.people ..
                    " Cargo: "    .. trainCfg.cargo
                elements[#elements + 1] = {
                    label = trainCfg.label,
                    value = train,
                    desc  = _U('price') .. trainCfg.price .. '<br>' .. _U('maxSpeed') .. trainCfg.maxSpeed .. '<br>' .. wagons,
                    info  = trainCfg,
                }
            end
        end
        if #elements <= 0 then
            allOwned = true
            elements = {{ label = _U('ownAllTrains'), value = 'noBuy', desc = '' }}
        end
    end
    ...
    OpenNUIMenu({
        ...
        onBack = function() MainMenu(station) end,
    })
end
        ...
        for myTrain, myTrainData in pairs(myTrains) do
            if myTrainData.trainModel == trainCfg.model then
                ...
                elements[#elements + 1] = {
                    label = trainCfg.label,
                    value = myTrain,
                    desc  = _U('maxSpeed') .. trainCfg.maxSpeed .. '<br>' .. _U('price') .. trainCfg.price .. '<br>' .. wagons,
                    info  = myTrainData,
                }
            end
        end
...
function SellMenu(myTrains, station)
    ...
    OpenNUIMenu({
        ...
        onSelect = function(data)
            ...
            if sold then MainMenu(station) else SellMenu(myTrains, station) end
        end,
        onBack = function() MainMenu(station) end,
    })
end
...
function DirectionMenu(trainCfg, myTrainData, station, myTrains)
    ...
    OpenNUIMenu({
        ...
        onBack = function() OwnedMenu(myTrains, station) end,
    })
end
