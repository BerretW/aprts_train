...
function DeliveryMission(station)
    ...
    for i, locationCfg in ipairs(Config.deliveryLocations) do
        if locationCfg.outWest == Stations[station].train.outWest then
            table.insert(locations, { index = i, cfg = locationCfg })
        end
    end
    local selected = locations[math.random(1, #locations)]
    destination = selected.cfg
    ...
    while InMission do
        ...
        if distance <= destination.radius then
            ...
            if Citizen.InvokeNative(0x78C3311A73135241, MyTrain) then -- IsVehicleStopped
                ...
                TriggerEvent('bcc-train:PlayerDelivery', destination, destinationIndex)
                break
            end
        end
...
AddEventHandler('bcc-train:PlayerDelivery', function(destination, destinationIndex)
    StartDeliveryPrompt()
    ...
    while InMission do
        ...
        if distance <= 2 and not IsPedDeadOrDying(PlayerPedId()) then
            sleep = 0
            ...
            if Citizen.InvokeNative(0xE0F65F0640EF0617, DeliveryPrompt) then -- PromptHasHoldModeCompleted
                ...
                MiniGame.Start('skillcheck', Config.minigame, function(result)
                    if result.passed then
                        ...
                        TriggerServerEvent('bcc-train:DeliveryPay', destinationIndex)
